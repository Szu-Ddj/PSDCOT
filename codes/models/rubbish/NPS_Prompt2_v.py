...
class NPS_prompt2_v(nn.Module):
    def __init__(self, bert,bert2, opt,tokenizer):
        super(NPS_prompt2_v, self).__init__()
        self.opt = opt
        ...
        self.fc_sk = nn.Linear(self.hid_dim,self.hid_dim)
        # self.standard_k = self.standard_k.to(self.bert.device)
        ...
        self.fc_out2 = nn.Linear(opt.n_views+3, 1)
        self.fc_target = nn.Linear(self.hid_dim,self.hid_dim)
        # self.att_pr = nn.ModuleList([MultiAttPreRea(self.hid_dim) for _ in range(opt.n_views+2)])
    ...
    def forward(self, inputs):
        ...
        # text_out = nn.Dropout(0.1)(text_out)
        ...
        #
        # _reason_out = reason_out.clone()
        # for rindex in range(reason_out.size(1)):
        #     # print( self.att_pr[rindex](reason_out[:,rindex,:], reason_out[:,rindex,:], reason_out[:,rindex,:]))
        #     reason_out[:,rindex,:],_ = self.att_pr[rindex](_reason_out[:,rindex,:], _reason_out[:,rindex,:], _reason_out[:,rindex,:])
        # #
        #Select rule
        ...
        reason_rules = torch.cat((reason_rules,target_rule),dim=1)
        # standard_k = torch.mean(reason_rules,dim=1,keepdim=True)
        out = reason_rules
        # out,_ = self.att_k(reason_rules,reason_rules,reason_rules)
        out = self.fc_out2(nn.GELU()(out.transpose(1,2))).squeeze(2)
